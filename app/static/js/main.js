...
// Function to run Python code for CFG page
...
// Function to run Python code for PDA page
...
// Function to run Python code for Parse Trees page
async function runPythonCodeParseTree() {
    const outputElement = document.getElementById('pythonCodeOutputParseTree');
    const codeEditor = window.AutomataEdu.codeEditorInstanceParseTree;
    const code = codeEditor.getValue();
    outputElement.textContent = 'Running code...';
...
// MCQ handling for PDA page

document.addEventListener('DOMContentLoaded', () => {
    const mcqContainerPDA = document.getElementById('mcq-container-pda');
    if (!mcqContainerPDA) return;

    const mcqQuestionsPDA = Array.from(mcqContainerPDA.querySelectorAll('.mcq-question-item'));
    let currentMcqIndexPDA = 0;
    const mcqProgressPDA = mcqContainerPDA.querySelector('.mcq-progress');

    function updateMcqDisplayPDA() {
        mcqQuestionsPDA.forEach((question, index) => {
            question.style.display = index === currentMcqIndexPDA ? 'block' : 'none';
        });
        if (mcqProgressPDA) {
            mcqProgressPDA.textContent = `Question ${currentMcqIndexPDA + 1} of ${mcqQuestionsPDA.length}`;
        }
    }

    function resetQuestionPDA(questionItem) {
        const options = questionItem.querySelectorAll('.mcq-option');
        options.forEach(option => {
            option.classList.remove('list-group-item-success', 'list-group-item-danger');
            option.disabled = false;
    ...
    mcqContainerPDA.addEventListener('click', (event) => {
        if (event.target.classList.contains('submit-mcq-btn')) {
            const questionItem = event.target.closest('.mcq-question-item');
            const selectedOption = questionItem.querySelector('.mcq-option.active');
            const correctAnswer = questionItem.dataset.correctAnswer;
...
// MCQ handling for Parse Trees page

document.addEventListener('DOMContentLoaded', () => {
    const mcqContainerParseTrees = document.getElementById('mcq-container-parse-trees');
    if (!mcqContainerParseTrees) return;

    const mcqQuestionsParseTrees = Array.from(mcqContainerParseTrees.querySelectorAll('.mcq-question-item'));
    let currentMcqIndexParseTrees = 0;
    const mcqProgressParseTrees = mcqContainerParseTrees.querySelector('.mcq-progress');

    function updateMcqDisplayParseTrees() {
        mcqQuestionsParseTrees.forEach((question, index) => {
            question.style.display = index === currentMcqIndexParseTrees ? 'block' : 'none';
        });
        if (mcqProgressParseTrees) {
            mcqProgressParseTrees.textContent = `Question ${currentMcqIndexParseTrees + 1} of ${mcqQuestionsParseTrees.length}`;
        }
    }

    function resetQuestionParseTrees(questionItem) {
        const options = questionItem.querySelectorAll('.mcq-option');
        options.forEach(option => {
            option.classList.remove('list-group-item-success', 'list-group-item-danger');
            option.disabled = false;
    ...
    mcqContainerParseTrees.addEventListener('click', (event) => {
        if (event.target.classList.contains('submit-mcq-btn')) {
            const questionItem = event.target.closest('.mcq-question-item');
            const selectedOption = questionItem.querySelector('.mcq-option.active');
            const correctAnswer = questionItem.dataset.correctAnswer;
    ...
    updateMcqDisplayParseTrees();
});

// Function to run Python code for Pumping Lemma page
async function runPythonCodePumpingLemma() {
    ...
    const code = codeEditor.getValue();
    // Always clear output before running
...
// MCQ handling for Pumping Lemma page

document.addEventListener('DOMContentLoaded', () => {
    const mcqContainerPumpingLemma = document.getElementById('mcq-container-pumping-lemma');
    if (!mcqContainerPumpingLemma) return;

    const mcqQuestionsPumpingLemma = Array.from(mcqContainerPumpingLemma.querySelectorAll('.mcq-question-item'));
    let currentMcqIndexPumpingLemma = 0;
    const mcqProgressPumpingLemma = mcqContainerPumpingLemma.querySelector('.mcq-progress');

    function updateMcqDisplayPumpingLemma() {
        mcqQuestionsPumpingLemma.forEach((question, index) => {
            question.style.display = index === currentMcqIndexPumpingLemma ? 'block' : 'none';
        });
        if (mcqProgressPumpingLemma) {
            mcqProgressPumpingLemma.textContent = `Question ${currentMcqIndexPumpingLemma + 1} of ${mcqQuestionsPumpingLemma.length}`;
        }
    }

    function resetQuestionPumpingLemma(questionItem) {
        const options = questionItem.querySelectorAll('.mcq-option');
        options.forEach(option => {
            option.classList.remove('list-group-item-success', 'list-group-item-danger');
            option.disabled = false;
    ...
    mcqContainerPumpingLemma.addEventListener('click', (event) => {
        if (event.target.classList.contains('submit-mcq-btn')) {
            const questionItem = event.target.closest('.mcq-question-item');
            const selectedOption = questionItem.querySelector('.mcq-option.active');
            const correctAnswer = questionItem.dataset.correctAnswer;
    ...
    updateMcqDisplayPumpingLemma();
});

// Function to run Python code for Turing Machines page
...
// MCQ handling for Turing Machines page

document.addEventListener('DOMContentLoaded', () => {
    const mcqContainerTM = document.getElementById('mcq-container-tm');
    if (!mcqContainerTM) return;

    const mcqQuestionsTM = Array.from(mcqContainerTM.querySelectorAll('.mcq-question-item'));
    let currentMcqIndexTM = 0;
    const mcqProgressTM = mcqContainerTM.querySelector('.mcq-progress');

    function updateMcqDisplayTM() {
        mcqQuestionsTM.forEach((question, index) => {
            question.style.display = index === currentMcqIndexTM ? 'block' : 'none';
        });
        if (mcqProgressTM) {
            mcqProgressTM.textContent = `Question ${currentMcqIndexTM + 1} of ${mcqQuestionsTM.length}`;
        }
    }

    function resetQuestionTM(questionItem) {
        const options = questionItem.querySelectorAll('.mcq-option');
        options.forEach(option => {
            option.classList.remove('list-group-item-success', 'list-group-item-danger');
            option.disabled = false;
    ...
    mcqContainerTM.addEventListener('click', (event) => {
        if (event.target.classList.contains('submit-mcq-btn')) {
            const questionItem = event.target.closest('.mcq-question-item');
            const selectedOption = questionItem.querySelector('.mcq-option.active');
            const correctAnswer = questionItem.dataset.correctAnswer;
...
// Function to run Python code for Decidability page
async function runPythonCodeDecidability() {
    const outputElement = document.getElementById('pythonCodeOutputDecidability');
    const codeEditor = window.AutomataEdu.codeEditorInstanceDecidability;
    const code = codeEditor.getValue();
    // Always clear output before running
    outputElement.textContent = '';
    outputElement.textContent = 'Running code...';
...
// MCQ handling for Decidability page

document.addEventListener('DOMContentLoaded', () => {
    const mcqContainerDecidability = document.getElementById('mcq-container-decidability');
    if (!mcqContainerDecidability) return;

    const mcqQuestionsDecidability = Array.from(mcqContainerDecidability.querySelectorAll('.mcq-question-item'));
    let currentMcqIndexDecidability = 0;
    const mcqProgressDecidability = mcqContainerDecidability.querySelector('.mcq-progress');

    function updateMcqDisplayDecidability() {
        mcqQuestionsDecidability.forEach((question, index) => {
            question.style.display = index === currentMcqIndexDecidability ? 'block' : 'none';
        });
        if (mcqProgressDecidability) {
            mcqProgressDecidability.textContent = `Question ${currentMcqIndexDecidability + 1} of ${mcqQuestionsDecidability.length}`;
        }
    }

    function resetQuestionDecidability(questionItem) {
        const options = questionItem.querySelectorAll('.mcq-option');
        options.forEach(option => {
            option.classList.remove('list-group-item-success', 'list-group-item-danger');
            option.disabled = false;
    ...
    mcqContainerDecidability.addEventListener('click', (event) => {
        if (event.target.classList.contains('submit-mcq-btn')) {
            const questionItem = event.target.closest('.mcq-question-item');
            const selectedOption = questionItem.querySelector('.mcq-option.active');
            const correctAnswer = questionItem.dataset.correctAnswer;
    ...
    updateMcqDisplayDecidability();
});

// Function to run Python code for LBA page
async function runPythonCodeLBA() {
    const outputElement = document.getElementById('pythonCodeOutputLBA');
    const codeEditor = window.AutomataEdu.codeEditorInstanceLBA;
    const code = codeEditor.getValue();
    // Always clear output before running
    outputElement.textContent = '';
    outputElement.textContent = 'Running code...';
...
// MCQ handling for Context-Sensitive Languages page

document.addEventListener('DOMContentLoaded', () => {
    const mcqContainerContextSensitive = document.getElementById('mcq-container-context-sensitive');
    if (!mcqContainerContextSensitive) return;

    const mcqQuestionsContextSensitive = Array.from(mcqContainerContextSensitive.querySelectorAll('.mcq-question-item'));
    let currentMcqIndexContextSensitive = 0;
    const mcqProgressContextSensitive = mcqContainerContextSensitive.querySelector('.mcq-progress');

    function updateMcqDisplayContextSensitive() {
        mcqQuestionsContextSensitive.forEach((question, index) => {
            question.style.display = index === currentMcqIndexContextSensitive ? 'block' : 'none';
        });
        if (mcqProgressContextSensitive) {
            mcqProgressContextSensitive.textContent = `Question ${currentMcqIndexContextSensitive + 1} of ${mcqQuestionsContextSensitive.length}`;
        }
    }

    function resetQuestionContextSensitive(questionItem) {
        const options = questionItem.querySelectorAll('.mcq-option');
        options.forEach(option => {
            option.classList.remove('list-group-item-success', 'list-group-item-danger');
            option.disabled = false;
    ...
    mcqContainerContextSensitive.addEventListener('click', (event) => {
        if (event.target.classList.contains('submit-mcq-btn')) {
            const questionItem = event.target.closest('.mcq-question-item');
            const selectedOption = questionItem.querySelector('.mcq-option.active');
            const correctAnswer = questionItem.dataset.correctAnswer;
    ...
    updateMcqDisplayContextSensitive();
});
